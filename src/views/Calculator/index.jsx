...
export default function Calculator() {
    ...
    const calculator = (key) => {
        if (!isNaN(key) || key === '.') {
            ...
            if (!currentOperation.value1 && !currentOperation.operation && !currentOperation.value2) {
                setCurrentOperation({ ...currentOperation, value1: key });
            } else if (currentOperation.value1 && !currentOperation.value2 && !currentOperation.operation) {
                setCurrentOperation({ ...currentOperation, value1: currentOperation.value1 + key });
            } else {
                setCurrentOperation({ ...currentOperation, value2: currentOperation.value2 + key });
            }
        } else {
            if(isOperator(key) && currentOperation.value1 && currentOperation.operation && currentOperation.value2) {
                getOperator(key);
            } else {
                if (key === '=') {
                    if (currentOperation.value1 === 0 && currentOperation.operation === '' && currentOperation.value2 === 0) {
                        return;
                    } else {
                        let result = getOperation(+currentOperation.value1, currentOperation.operation, +currentOperation.value2);
                        setCalcStorageDisplay(result);
                    }
                } else {
                    if (key === 'D') {
                    } else {
    ...
    const getOperator = (newOperation) => {
        const displayValue = getOperation(+currentOperation.value1, currentOperation.operation, +currentOperation.value2);
        setCurrentOperation({ value1: displayValue, operation: newOperation, value2: '' });
